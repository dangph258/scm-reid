...
def run_single(cfg, method, logger_save_dir):
    ...
    logger = TensorBoardLogger(cfg.LOG_DIR, version='')
    ...
    dm_dest = init_dataset(
        cfg.DATASETS.DEST, cfg=cfg, num_workers=cfg.DATALOADER.NUM_WORKERS
    )
    dm_dest.setup()
    val_dataloader_dest=dm_dest.val_dataloader()
    ...
    if cfg.TEST.ONLY_TEST:
        ...
        print("---Training on {} and testing on {}".format(cfg.DATASETS.NAMES, cfg.DATASETS.NAMES))
        trainer.test(model=method, test_dataloaders=val_dataloader)
        
        print("---Training on {} and testing on {}".format(cfg.DATASETS.NAMES, cfg.DATASETS.DEST))
        trainer.test(model=method, test_dataloaders=val_dataloader_dest)
...
def run_main(cfg, method, logger_save_dir):
    ...
    if cfg.REPRODUCIBLE:
        ...
        for _ in range(0, cfg.REPRODUCIBLE_NUM_RUNS):
            start = time.time()
            run_single(cfg, method, logger_save_dir)
            print("---Producible time: ", time.time() - start)
