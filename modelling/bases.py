...
class ModelBase(pl.LightningModule):
    def __init__(self, cfg=None, test_dataloader=None, **kwargs):
        ...
        self.focal = FocalLoss(gamma=2.0, alpha=None)

        self.losses_names = ["query_xent", "query_triplet", "query_center", "query_focal", "centroid_triplet", "supcon_loss"]
    ...
    @rank_zero_only
    def get_val_metrics(self, embeddings, labels, camids, respect_camids=False):
        ...
        for top_k, kk in zip(all_topk, [1, 5, 10, 20, 50]):
            print("top-k, Rank-{:<3}:{:.3%}".format(kk, top_k))
    ...
    @rank_zero_only
    def get_val_metrics_respect_camids(self, embeddings, labels, camids, respect_camids=True):
        self.r1_map_func = R1_mAP(
            pl_module=self,
            num_query=self.hparams.num_query,
            feat_norm=self.hparams.TEST.FEAT_NORM,
        ...
        for top_k, kk in zip(all_topk, [1, 5, 10, 20, 50]):
            print("top-k, Rank-{:<3}:{:.3%}".format(kk, top_k))
            topks[f"Top-{kk}-camids"] = top_k
        print(f"mAP-camids: {mAP}")

        log_data = {"mAP-camids": mAP}
    ...
    @rank_zero_only
    def get_val_metrics_ctl(self, embeddings, labels, camids, respect_camids=True):
        self.r1_map_func = R1_mAP(
            pl_module=self,
            num_query=self.hparams.num_query,
            feat_norm=self.hparams.TEST.FEAT_NORM,
        ...
        for top_k, kk in zip(all_topk, [1, 5, 10, 20, 50]):
            print("top-k, Rank-{:<3}:{:.3%}".format(kk, top_k))
            topks[f"Top-{kk}-ctl"] = top_k
        print(f"mAP-ctl: {mAP}")

        log_data = {"mAP-ctl": mAP}
    ...
    def validation_epoch_end(self, outputs):
        if self.trainer.global_rank == 0 and self.trainer.local_rank == 0:
            ...
            print("Evaluation DONOT use CENTROIDS")
            if self.trainer.global_rank == 0 and self.trainer.local_rank == 0:
                self.get_val_metrics(embeddings, labels, camids)
                # self.get_val_metrics_respect_camids(embeddings, labels, camids) 
                    
            
            print("Evaluation do USE centroids")
            #reduce search space, ex: from 15913 to 6686 id.
            ...
            if self.trainer.global_rank == 0 and self.trainer.local_rank == 0:
                    self.get_val_metrics_ctl(embeddings, labels, camids)
    ...
    def eval_on_train(self):
        ...
        if self.trainer.global_rank == 0 and self.trainer.local_rank == 0:
            ...
            topks = {}
            for top_k, kk in zip(all_topk, [1, 5, 10, 20, 50]):
                print("Train top-k, Rank-{:<3}:{:.3%}".format(kk, top_k))
    ...
    #     if batch_idx > 115:  # Validate every 10 steps
