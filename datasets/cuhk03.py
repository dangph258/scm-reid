...
class CUHK03(ReidBaseDataModule):
    ...
    def __init__(self, cfg, split_id=0, cuhk03_labeled=False, cuhk03_classic_split=False, **kwargs):
        ...
        # time.sleep(2)
        cuhk03_labeled = cfg.DATASETS.LABELED
    ...
    def setup(self):
        ...
        assert self.split_id < len(splits), "Condition split_id ({}) < len(splits) ({}) is false".format(self.split_id,
                                                                                                    len(splits))
        split = splits[self.split_id]
        print("Split index = {}".format(self.split_id))
        ...
        train, train_dict = self._process_dataset_dict(train)
        self.train_dict = train_dict
        self.train_list = train
        query, query_dict = self._process_dataset_dict(query)
        gallery, gallery_dict  = self._process_dataset_dict(gallery)
    ...
    def _process_dataset_dict(self, train):
        dataset = []
        dataset_dict = defaultdict(list)
        idx = 0
        for path, pid, camid in train:
            idx += 1
            dataset.append((path, pid, camid, idx))
            dataset_dict[pid].append((path, pid, camid, idx))
