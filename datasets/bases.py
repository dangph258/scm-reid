...
class ReidBaseDataModule(pl.LightningDataModule):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.num_instances = self.cfg.DATALOADER.NUM_INSTANCE
        # self.num_instances = (
...
class BaseDatasetLabelledPerPid(Dataset):
    ...
    def __getitem__(self, pid):
        ...
        for _ in range(choice_size):
            ...
            img1 = self.prepare_img(path)
            out.append(
                (img, img1, target, camid, idx, True)
        ...
        if needPad:
            ...
            if self.resample:
                ...
                for idx in resampled:
                    ...
                    img = self.prepare_img(path)
                    img1 = self.prepare_img(path)
                    out.append((img, img1, target, camid, idx, True))
            else:
                img_mock = torch.zeros_like(img)
                for _ in range(num_missing):
                    out.append((img_mock, img_mock, target, camid, idx, False))
...
def collate_fn_alternative(batch):
    ...
    imgs1 = [item[1] for sample in batch for item in sample]
    pids = [item[2] for sample in batch for item in sample]
    camids = [item[3] for sample in batch for item in sample]
    isReal = [item[5] for sample in batch for item in sample]

    pids = torch.tensor(pids, dtype=torch.int64)
    return torch.stack(imgs, dim=0), torch.stack(imgs1, dim=0), pids, torch.tensor(camids), torch.tensor(isReal)
